...
class splash : AppCompatActivity() {
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        if(!hasPermission(this@splash, *permissionStrings)){
            // Asking for permissions
            ActivityCompat.requestPermissions(this@splash,permissionStrings,131) // Any arbitrary requestCode
        }
        else{
        ...
        when(requestCode){
            131 ->{
                        ...
                        && grantResults[4]==PackageManager.PERMISSION_GRANTED)
                {
                    ...
                    Toast.makeText(this@splash,"Grant Permissions", Toast.LENGTH_SHORT).show()
                    this.finish()
                }
                return
            }
